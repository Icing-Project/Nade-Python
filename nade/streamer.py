...
class AudioStreamer:
    ...
    def __init__(self, 
        ...
        if pyaudio is None:
            self.logger.error("PyAudio not installed. Cannot initialize stream.")
            self.stream = None
            self.pa = None
            # raise ImportError("PyAudio is required for AudioStreamer. Install with 'pip install nade-python[audio]'")
            return
